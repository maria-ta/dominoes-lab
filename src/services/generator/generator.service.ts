...
export class GeneratorService {
  ...
  private placeCard(card: number[], position: number[]) {
    console.log(`Place card on position ${position}`);
    const [i, j] = position;
    ...
    if (optionsWithOnlyOnePossibleOptions.length > 1) {
      ...
      console.log(`There is one option which should be used now`);
      this.placeCardOn(card, position, availableOptions[0]);
    } else if (availableOptions.length > 0) {
      console.log(`There are multiple options which could be used now`);
      const randomPosition = this.getRandomElementFromAnArray(availableOptions);
      console.log(`Random option is ${randomPosition}`);
  ...
  private placeCardOn(
    ...
    console.log(`Place card on [${x}, ${y}], [${i}, ${j}]`);
